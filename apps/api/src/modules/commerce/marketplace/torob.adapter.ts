...
const NOT_IMPLEMENTED_MESSAGE = "Torob production integration is not yet implemented (no official merchant documentation available).";

/**
 * ============================================================================
 * PROVIDER DOCUMENTATION SAFETY — Torob (spec sections 19, 58)
 * ============================================================================
 * Official docs source:        UNKNOWN — no Torob merchant/API documentation
 *                               was available to this project at
 *                               implementation time.
 * Auth mechanism:               UNKNOWN.
 * Sandbox availability:         UNKNOWN.
 * Required credentials:         UNKNOWN — `TOROB_BASE_URL`/`TOROB_API_KEY`
 ...
 * Order pull/webhooks:          UNKNOWN — capabilities below mark
 *                               supportsOrderPull/supportsWebhooks as false
 *                               (see marketplace-provider-registry.ts) since
 *                               this project could not confirm Torob
 *                               exposes an order feed to merchants at all;
 *                               `fetchOrders`/`verifyWebhook` below exist
 *                               only for interface completeness and are
 *                               never called by MarketplaceChannelRegistry
 *                               for this provider (assertCapability blocks it).
 ...
 * real Torob endpoint, invent a request/response shape, or claim a
 * production integration. Every method below delegates to the same generic,
 * clearly-labeled simulation engine DevMarketplaceAdapter uses (see
 * marketplace-simulation.util.ts) — this proves the SalesChannelAdapter/
 * registry/orchestrator boundary genuinely supports a third provider
 * without any Seller OS/Offer/Inventory code change (spec section 1's core
 * requirement), while never presenting a simulated field/status name as a
 * real Torob API value. Replacing this file's internals with a real HTTP
 * client is the only change a credentialed integration would need.
 * ============================================================================
 */
@Injectable()
export class TorobAdapter implements SalesChannelAdapter {
  readonly provider = MarketplaceProvider.TOROB;
  readonly capabilities = MARKETPLACE_PROVIDER_CAPABILITIES[MarketplaceProvider.TOROB];
  ...
  private isProductionConfigured(): boolean {
    return this.config.get("MARKETPLACE_SANDBOX_MODE", { infer: true }) === "production" && !!this.config.get("TOROB_BASE_URL", { infer: true }) && !!this.config.get("TOROB_API_KEY", { infer: true });
  }
  ...
  async publishListing(input: PublishListingInput): Promise<PublishListingResult> {
    ...
    const result = simulatePublishListing(input, "torob");
  ...
  /** Never called — supportsOrderPull is false for Torob (see class doc comment). */
  ...
  /** Never called — supportsWebhooks is false for Torob (see class doc comment). */
