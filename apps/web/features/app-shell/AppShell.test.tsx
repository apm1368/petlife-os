...
describe("AppShell bootstrap recovery", () => {
  ...
  it("does not render incomplete household context after a successful session request", async () => {
    vi.mocked(authService.getSession).mockResolvedValue({ user: {
      id: "u1", email: "sarah@example.com", phone: null, displayName: "Sarah",
      avatarUrl: null, locale: "en", themePreference: "SYSTEM", createdAt: "", updatedAt: "",
    } });
  ...
  it("still sends an unauthenticated visitor to auth in their current locale", async () => {
    ...
    await waitFor(() => expect(replace).toHaveBeenCalledWith("/fa/welcome"));
