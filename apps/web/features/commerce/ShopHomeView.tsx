...
import { ContextSurface, Skeleton } from "@petlife/ui";
...
export function ShopHomeView() {
  ...
  useEffect(() => {
    void commerceService.listCategories().then(setCategories);
  }, []);

  useEffect(() => {
    setProducts(null);
    void commerceService.searchProducts({ petId: activePet?.id }).then(setProducts);
  }, [activePet?.id]);
        ...
        {!categories ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            {categories.map((category) => (
              <button key={category.id} type="button" className="text-start" onClick={() => router.push(`/${locale}/shop/products?category=${category.id}`)}>
