...
export function ProductNavigation({ audience = "public" }: { audience?: "public" | "consumer" }) {
  ...
  if (audience === "consumer") return <nav aria-label={locale === "fa" ? "بخش‌های محصول" : "Product sections"} className="flex flex-wrap gap-1 border-b border-border-subtle px-4 py-2">{consumerDestinations.map(([path, fa, en]) => <Link key={path} href={`/${locale}${path}`} aria-current={!path.includes("?") && (pathname === path || (path !== "/home" && pathname.startsWith(`${path}/`) && !consumerDestinations.some(([other]) => other !== path && !other.includes("?") && other.startsWith(`${path}/`) && (pathname === other || pathname.startsWith(`${other}/`))))) ? "page" : undefined} className="rounded-full px-3 py-2 text-metadata text-text-primary hover:bg-surface-subtle focus-visible:outline focus-visible:outline-2">{locale === "fa" ? fa : en}</Link>)}</nav>;
